...
fun Context.getOptimalThumbnailSize(): Pair<Int, Int> {
    ...
    val availableMemory = runtime.availableMemoryMB
    return when {
...
fun Context.shouldPreloadThumbnails(): Boolean {
    val runtime = Runtime.getRuntime()
    return runtime.availableMemoryMB > 100
}
...
fun optimizeMemoryUsage(context: Context) {
    try {
        val runtime = Runtime.getRuntime()
        val usageRatio = runtime.memoryUsageRatio
...
fun Context.getPerformanceStats(): Map<String, String> {
    return try {
        ...
        mapOf(
            ...
            "available_memory_mb" to "${runtime.availableMemoryMB}",
